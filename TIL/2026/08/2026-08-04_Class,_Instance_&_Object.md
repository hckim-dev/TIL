...
> 날짜: 2026-08-04
> 원본 노션: [링크](https://app.notion.com/p/Class-Instance-Object-3b11d46c18fc808aab32e093aca21d93)
...
- Object (객체): 현실 세계에 존재하는 사물, 현상, 개념 등 속성(상태)과 동작(기능)을 가진 모든 것.
- Abstraction (추상화): 객체들의 공통적인 특징(속성, 기능)을 뽑아내어 코드로 정의하는 과정.
- Class (클래스): 추상화를 통해 만들어진 설계도(틀). (C++에서는 struct도 클래스와 동일한 역할을 수행)
- Instance (인스턴스): 클래스(설계도)를 바탕으로 메모리에 실제로 생성된 실체(객체).
## 3. C++에서의 struct와 class

C++은 C의 struct를 확장하여 내부에 함수(메서드)를 포함하고 접근 지정자를 사용할 수 있도록 만들었습니다.

- C++에서 struct와 class는 기능적으로 99% 동일합니다.
...
- private: 클래스(구조체) 내부 멤버 함수에서만 접근 가능 (데이터 보호).
- public: 클래스 외부(예: main() 함수 등) 어디서나 접근 가능 (인터페이스 제공).
- protected: 상속 관계의 자식 클래스까지 접근 허용 (상속 다뤄질 때 사용).
...
1. static 멤버 변수: 인스턴스 생성 여부와 상관없이 프로그램 전체에서 단 1개만 존재하는 클래스 변수 (클래스 외부에 int A::s = 0; 형태로 반드시 정의 필요).
1. static 멤버 함수: 인스턴스 없이 A::fc()로 호출 가능하며, static이 아닌 일반 멤버 변수/함수에는 접근 불가능.
1. 멤버 초기화 리스트: const 멤버 변수나 참조자는 생성자 괄호 {} 안에서 대입할 수 없으므로 반드시 초기화 리스트(: age(a))를 사용.
1. const 멤버 함수: int func() const 형태로 작성하며, 함수 내부에서 클래스 멤버 변수의 값을 변경하는 것을 금지함.
```c++
#include <iostream>
using namespace std;
...
1. 클래스 템플릿 (template <typename T>): 다양한 데이터 타입을 하나의 클래스 구조로 다룰 수 있게 해주는 기능 (인스턴스 생성 시 mart_calc<int> a; 처럼 타입 명시 필요).
1. 클래스 멤버 포인터 (int A::*pp = &A::p;): 특정 객체의 주소가 아닌, 클래스 내부에서 해당 변수가 위치한 상대적 오프셋(Offset)을 저장하는 포인터.
1. .* 연산자: 객체와 멤버 포인터를 조합하여 실제 메모리 값에 접근할 때 사용 (a.*pp).
```c++
#include <iostream>
using namespace std;
